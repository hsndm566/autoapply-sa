...
1. **Never send an application email directly through SMTP, Gmail, a browser, or any custom script.** Every email application must use `email_dispatcher.queue_audited_email_application()` and `email_dispatcher.dispatch_one()` / `dispatch_pending()`.
   Dispatcher-internal Brevo delivery may use the authorized sender pool (`apply@hsndm.tech`, `apply1@hsndm.tech`, `apply2@hsndm.tech`). These addresses are reusable sender identities and must never be permanently bound to a person or client ID in source code. Candidate identity comes from the current client/application package and must match the package sender email at audit time. The one-time verified-contact scope requires its explicit environment gate, while scheduled verified-contact delivery requires `AUTOAPPLY_SCHEDULED_DELIVERY=true`. Both scopes remain limited to verified-contact records, configured client identities, Auditor-approved exact PDF attachments, MX validation, tracking deduplication, randomized delays, a five-per-identity cap, and no automatic retry after a transport-uncertain result.
