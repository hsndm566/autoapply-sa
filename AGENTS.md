...
1. **Never send an application email directly through SMTP, Gmail, a browser, or any custom script.** Every email application must use `email_dispatcher.queue_audited_email_application()` and `email_dispatcher.dispatch_one()` / `dispatch_pending()`.
   The only authorized transport exceptions are dispatcher-internal Brevo paths for client 2 (`apply1@hsndm.tech`) and client 3 (`apply2@hsndm.tech`): the completed one-time `verified-contact-clients-2-3-2026-08-20` scope requires its explicit one-time environment gate, while the schedule-only `verified-contact-clients-2-3-scheduled` scope requires `AUTOAPPLY_SCHEDULED_DELIVERY=true`. Both scopes remain limited to verified-contact records, active client identities, Auditor-approved exact PDF attachments, MX validation, tracking deduplication, randomized delays, a five-per-identity cap, and no automatic retry after a transport-uncertain result.
