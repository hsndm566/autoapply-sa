#!/usr/bin/env python3
"""Human-approved Ashby submission adapter."""
...
@requires_approval
def submit_ashby(rec: dict[str, Any], candidate_data: dict[str, Any], *, session=None) -> dict[str, Any]:
    if rec.get("_path") != "portal_upload_verified":
        raise PermissionError("Ashby submission requires portal_upload_verified")
    ...
    try:
        with sync_playwright() as p:
            ...
            _fill(page, ['input[name="_systemfield_name"]', 'input[name*="name"]'], full_name)
            _fill(page, ['input[name="_systemfield_email"]', 'input[name*="email"]'], str(candidate_data.get("email") or ""))
            _fill(page, ['input[name="_systemfield_phone"]', 'input[name*="phone"]'], str(candidate_data.get("phone") or ""))
            ...
            submitted_rec = mark_submitted(rec, evidence=evidence, channel="ashby")
