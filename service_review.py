#!/usr/bin/env python3
"""AutoApply API service with campaign-scoped human review routes.

This adapter subclasses the existing service handler instead of duplicating the
large campaign API. All pre-existing endpoints continue to use ``service.py``;
only the review routes are intercepted here.
"""
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if len(parts) == 7 and parts[:2] == ["v1", "campaigns"] and parts[3] == "review":
            ...
            try:
                ...
                LOG.warning("review runtime unavailable: %s", type(exc).__name__)
